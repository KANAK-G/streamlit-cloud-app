"""Data product overview — KPI dashboard over the Vulcan API."""
...
# Optional env default; leave empty so users set URL in the sidebar.
DEFAULT_BASE_URL = ""

# Kinds from /metadata/semantic — everything else in /metadata is treated as catalog/physical.
SEMANTIC_LAYER_KINDS = {"SEMANTIC", "METRIC"}


st.set_page_config(
    page_title="Data Product Dashboard",
    page_icon="📊",
    layout="wide",
...
def build_vulcan_base_url(instance: str, tenant: str, product: str, path_style: str) -> str:
    """Compose a Vulcan API base URL from DataOS parts (no trailing /api/v1)."""
    instance = instance.strip().removesuffix(".dataos.cloud")
    tenant = tenant.strip().strip("/")
    product = product.strip().strip("/")
    if not instance or not tenant or not product:
        return ""
    host = instance if "." in instance else f"{instance}.dataos.cloud"
    if path_style == "platform":
        return f"https://{host}/platform/data-product/{tenant}/{product}"
    return f"https://{host}/{tenant}/vulcan/{product}"


def is_physical_model(model: Dict[str, Any]) -> bool:
    """Catalog/physical models from /metadata (includes Spark EXTERNAL, dbt FULL, etc.)."""
    kind = str(model.get("kind", "")).upper()
    if not kind:
        return True
    return kind not in SEMANTIC_LAYER_KINDS


def physical_kind_summary(metadata: Dict[str, Any], limit: int = 3) -> str:
    from collections import Counter

    kinds = Counter(
        str(m.get("kind", "—")).upper()
        for m in safe_list(metadata.get("models"))
        if isinstance(m, dict) and is_physical_model(m)
    )
    if not kinds:
        return "Catalog models"
    return ", ".join(f"{k} ({n})" for k, n in kinds.most_common(limit))
...
def count_physical_models(metadata: Dict[str, Any]) -> int:
    return sum(
        ...
        if isinstance(model, dict) and is_physical_model(model)
    )
...
def physical_models_df(metadata: Dict[str, Any]) -> pd.DataFrame:
    rows = []
    for model in safe_list(metadata.get("models")):
        if not isinstance(model, dict) or not is_physical_model(model):
            continue
        kind = str(model.get("kind", "")).upper() or "—"
        rows.append(
            {
                "Model": model.get("name", "—"),
                "Kind": kind,
                "Source": model.get("source_type", "—"),
...
with st.sidebar:
    ...
    st.caption("Vulcan API base URL (without `/api/v1`).")

    with st.expander("Build URL from parts"):
        instance = st.text_input(
            "Instance",
            value=get_config("DATAOS_INSTANCE", "pacific-051426"),
            help="e.g. pacific-051426 or lnv-051826",
        )
        tenant = st.text_input(
            "Tenant",
            value=get_config("VULCAN_TENANT", "product-sandbox"),
            help="e.g. product-sandbox, sandbox",
        )
        product = st.text_input(
            "Data product",
            value=get_config("VULCAN_DATA_PRODUCT", ""),
            help="e.g. orders360-vdetrue, device360sadp",
        )
        path_style = st.selectbox(
            "Path style",
            options=["vulcan", "platform"],
            format_func=lambda x: (
                "/{tenant}/vulcan/{product}"
                if x == "vulcan"
                else "/platform/data-product/{tenant}/{product}"
            ),
            index=0 if get_config("VULCAN_PATH_STYLE", "vulcan") != "platform" else 1,
        )
        if st.button("Use built URL", use_container_width=True):
            built = build_vulcan_base_url(instance, tenant, product, path_style)
            if built:
                st.session_state["vulcan_base_url"] = built
            else:
                st.warning("Fill instance, tenant, and data product name.")

    default_url = st.session_state.get("vulcan_base_url", base_url)
    base_url = st.text_input(
        "Vulcan API base URL",
        value=default_url,
        placeholder="https://{instance}.dataos.cloud/{tenant}/vulcan/{data-product}",
        help="Paste a full URL or use **Build URL from parts** above.",
    )
    st.session_state["vulcan_base_url"] = base_url
...
if not base_url.strip():
    st.warning("Set the **Vulcan API base URL** in the sidebar (or `VULCAN_BASE_URL` in the environment).")
elif not token:
    st.warning("Add your DataOS API token in the sidebar (or pass `VULCAN_AUTH_TOKEN` when starting Docker).")

payload = load_dashboard_data(base_url, token) if base_url.strip() else {"data": {}, "errors": {}}
data = payload["data"]
...
if errors:
    st.error(
        f"Could not load live data from the Vulcan API ({len(errors)} endpoint(s) failed). "
        "Set a valid token in the sidebar, then click **Refresh data**."
    )
    with st.expander("API error details", expanded=True):
        for name, msg in errors.items():
            st.code(f"{name}: {msg}", language=None)

# --- KPI row ---
c1, c2, c3, c4, c5, c6, c7, c8 = st.columns(8)
with c1:
    kpi_card("Physical models", physical_count, physical_kind_summary(metadata))
with c2:
